...
for expert=1:length(experts_list)
    ...
    for cnt=1:150
        ...
        * 2 * 4 * 6 * 8 * ...
        ...
        bar_color = zeros(1,length(cut_line)-1);
        for j=1:length(cut_line)-1
            pos = find(Img(1:660,cut_line(j)+9)~=255, 1);
            if Imgrgb(pos, cut_line(j)+9, 1) > Imgrgb(pos, cut_line(j)+9, 3)  % Red > Blue?
                bar_color(j) = 1;
            else
                bar_color(j) = 3;
            end
        ...
        red_ratio = red_num /(red_num+blue_num);
        experts_red_ratio((expert-1)*150+cnt) = red_ratio;
    end
end

clearvars -except experts_red_ratio;
...
for nexpert=1:length(nexperts_list)
    ...
    for cnt=1:150
        ...
        * 2 * 4 * 6 * 8 * ...
        ...
        red_ratio = red_num /(red_num+blue_num);
        nexperts_red_ratio((nexpert-1)*150+cnt) = red_ratio;
    end
end