...
for expert=1:length(experts_list)
    ...
    for cnt = 1:150
        ...
        * 2 * 4 * 6 * 8 * ...
        ...
        graph_num = zeros(1,max(graph_unique));
        graph_adj = zeros(length(graph_unique), length(graph_unique));
        for j=1:length(graph_unique)
            graph_num(graph_unique(j)) = j;
        end

        Order = length(graph_unique);
        Edge = length(graph_order) - 1;
        for fix_p=2:length(graph_order)
            ...
            graph_adj(graph_num(start_vert), graph_num(end_vert)) = len;
            graph_adj(graph_num(end_vert), graph_num(start_vert)) = len;
        end

        graph_deg = diag(sum(graph_adj));
        graph_lap = graph_deg - graph_adj;

        graph_lap_eig = eig(graph_lap);
        LE = sum(abs(graph_lap_eig - 2*Edge/Order));
        experts_LE(150*(expert - 1)+cnt) = LE;
    end
end

%%
clearvars -except experts_LE;
...
for nexpert=1:length(nexperts_list)
    ...
    for cnt = 1:150
        ...
        * 2 * 4 * 6 * 8 * ...
        ...
        graph_lap_eig = eig(graph_lap);
        LE = sum(abs(graph_lap_eig - 2*Edge/Order));
        nexperts_LE(150*(nexpert - 1)+cnt) = LE;
    end
end